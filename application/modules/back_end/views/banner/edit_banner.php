<!-- Main Content -->
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <div class="section-header-breadcrumb">
                <div class="section-header-breadcrumb">
                    <div class="breadcrumb-item"><a href="<?php echo base_url('backoffice/dashboard'); ?>">แดชบอร์ด</a></div>
                    <div class="breadcrumb-item"><a href="#">หน้า: หน้าแรก</a></div>
                    <div class="breadcrumb-item"><a href="<?php echo base_url('backoffice/page/home/banner'); ?>">แบนเนอร์</a></div>
                    <div class="breadcrumb-item active">แก้ไข แบนเนอร์</div>

                </div>
            </div>
        </div>

        <div class="section-body">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>แก้ไข แบนเนอร์</h4>
                        </div>
                        <div class="card-body">
                            <form action="<?php echo base_url('backoffice/page/home/banner/update/'.$banners->id); ?>" method="post"
                                  enctype="multipart/form-data">
                                <div style="padding-top: 20px;"><h6><i class="fa fa-tag"></i> ข้อมูล</h6></div>
                                <hr>
                                <div class="form-group row">
                                    <label class="col-sm-3 col-form-label">พรีวิว</label>
                                    <div class="col-sm-9">
                                        <img id="previewImgCover" src="<?php echo base_url('storage/uploads/images/banner/'. $banners->title );?>" style="background-color: #fff;" width="20%"
                                             onerror="this.onerror=null;this.src='data:image/png;base64,';">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-sm-3 col-form-label">* เลือกภาพ <span style="color: #c0372b;">(แนะนำขนาด 1000*500)</span></label>
                                    <div class="col-sm-9">
                                        <div class="custom-file input-file-w-30">
                                            <input type="file" class="custom-file-input" name="img_cover"
                                                   onchange="PreviewImage(this, 'previewImgCover');" value="<?php echo $banners->title; ?>" required  accept="image/gif, image/jpeg, image/png">
                                            <label class="custom-file-label">เลือกไฟล์ (.gif | .jpg | .png)</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-sm-3 col-form-label">ชื่อภาพ</label>
                                    <div class="col-sm-9">
                                        <input type="text" class="form-control" name="img_title_alt" value="<?php echo $banners->img_title_alt; ?>">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-sm-3 col-form-label">URL ภาพ</label>
                                    <div class="col-sm-9">
                                        <input type="text" class="form-control" name="img_url" value="<?php echo $banners->img_url; ?>">
                                    </div>
                                </div>

                                <div class="text-right">
                                    <button type="submit" class="btn btn-primary">บันทึก</button>
                                    <a href="<?php echo base_url('backoffice/page/home/banner');?>" class="btn btn-secondary">ปิด</a>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>


<script>
    function PreviewImage(input, previewImage) {
        if (input.files && input.files[0]) {
            var reader = new FileReader()

            reader.onload = function (e) {
                $('#' + previewImage).attr('src', e.target.result);
            }

            reader.readAsDataURL(input.files[0]);
        }
    }
</script>
